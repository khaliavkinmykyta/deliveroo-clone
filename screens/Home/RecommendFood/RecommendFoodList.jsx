...
const RecommendFoodList = () => {
    ...
    useEffect(() => {
        const fetchRecommendFoodItems = async () => {
            try {
              const recommendFoodRef = collection(db, 'recommendFoodItem');
              const querySnapshot = await getDocs(recommendFoodRef);
  ...
  return (
    <View>
      ...
      <Text className="text-xs px-2 text-gray-500">Топ-10 позицій за цей тиждень</Text>
      <ScrollView 
        horizontal
